...
require_once WCPG_PLUGIN_DIR . 'includes/class-semantic-tables.php';
...
/**
 * Theme integration points. Guard call sites with function_exists() so the
 * theme works without the plugin.
 */
function wcpg_connections_panel($post_id) {
    WCPG_Connections_Panel::instance()->render((int) $post_id);
}

/** Render the semantic tables for a page ($heading_id = 0) or a heading section. */
function wcpg_render_tables($page_id, $heading_id = 0) {
    WCPG_Semantic_Tables::instance()->render((int) $page_id, (int) $heading_id);
}
