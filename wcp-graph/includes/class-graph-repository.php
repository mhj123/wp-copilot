...
class WCPG_Graph_Repository {
     ...
     * stored ones first, then derived `contains`/`within` edges read from
     * the page/heading/item structure.
     */
    public function edges_for_post($post_id) {
        ...
        $derived         = WCPG_Structure::instance()->derived_edges_for($post_id);
        $out['outbound'] = array_merge($out['outbound'], $derived['outbound']);
        $out['inbound']  = array_merge($out['inbound'], $derived['inbound']);

        return $out;
    }

    /**
     * All edges for a set of subjects across a set of predicates, keyed
     * "{subject_id}:{predicate_id}" — one query fills a semantic table.
     */
    public function edges_matrix($subject_ids, $predicate_ids) {
        global $wpdb;
        $table = self::table();

        $subject_ids   = array_filter(array_map('intval', (array) $subject_ids));
        $predicate_ids = array_filter(array_map('intval', (array) $predicate_ids));
        if (empty($subject_ids) || empty($predicate_ids)) {
            return array();
        }

        $s_in = implode(',', array_fill(0, count($subject_ids), '%d'));
        $p_in = implode(',', array_fill(0, count($predicate_ids), '%d'));

        $rows = $wpdb->get_results($wpdb->prepare(
            "SELECT * FROM {$table} WHERE subject_id IN ({$s_in}) AND predicate_id IN ({$p_in}) ORDER BY created_at ASC",
            array_merge($subject_ids, $predicate_ids)
        ));
        ...
        foreach ($rows as $row) {
            ...
            $matrix[$key][] = $this->format_edge($row);
        }
        return $matrix;
    }
