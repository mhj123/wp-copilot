...
class WCPG_REST_API {
    ...
    public function register_routes() {
        ...
        // Semantic table config (view definitions only — cells are edges)
        register_rest_route($namespace, '/pages/(?P<page_id>\d+)/tables', array(
            'methods'             => 'POST',
            'callback'            => array($this, 'create_table'),
            'permission_callback' => array($this, 'check_permission'),
        ));

        register_rest_route($namespace, '/pages/(?P<page_id>\d+)/tables/(?P<table_id>[a-zA-Z0-9_]+)', array(
            'methods'             => 'DELETE',
            'callback'            => array($this, 'delete_table'),
            'permission_callback' => array($this, 'check_permission'),
        ));

        register_rest_route($namespace, '/pages/(?P<page_id>\d+)/tables/(?P<table_id>[a-zA-Z0-9_]+)/columns', array(
            'methods'             => 'POST',
            'callback'            => array($this, 'add_table_column'),
            'permission_callback' => array($this, 'check_permission'),
        ));

        register_rest_route($namespace, '/pages/(?P<page_id>\d+)/tables/(?P<table_id>[a-zA-Z0-9_]+)/columns/(?P<predicate_id>\d+)', array(
            'methods'             => 'DELETE',
            'callback'            => array($this, 'remove_table_column'),
            'permission_callback' => array($this, 'check_permission'),
        ));
    ...
    public function create_table($request) {
        $result = WCPG_Semantic_Tables::instance()->create_table(
            (int) $request->get_param('page_id'),
            (int) $request->get_param('heading_id'),
            (string) $request->get_param('columns')
        );
        if (is_wp_error($result)) {
            return $result;
        }
        return rest_ensure_response(array('success' => true, 'table' => $result));
    }

    public function delete_table($request) {
        $result = WCPG_Semantic_Tables::instance()->delete_table(
            (int) $request->get_param('page_id'),
            sanitize_key($request->get_param('table_id'))
        );
        if (is_wp_error($result)) {
            return $result;
        }
    ...
    public function add_table_column($request) {
        $result = WCPG_Semantic_Tables::instance()->add_column(
            (int) $request->get_param('page_id'),
            sanitize_key($request->get_param('table_id')),
    ...
    public function remove_table_column($request) {
        $result = WCPG_Semantic_Tables::instance()->remove_column(
            (int) $request->get_param('page_id'),
            sanitize_key($request->get_param('table_id')),
            (int) $request->get_param('predicate_id')
        );
        if (is_wp_error($result)) {
            return $result;
        }
