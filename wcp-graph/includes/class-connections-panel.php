...
class WCPG_Connections_Panel {
    ...
    public function enqueue_assets() {
        ...
        if (is_page()) {
            wp_enqueue_script('wcpg-tables', WCPG_PLUGIN_URL . 'assets/js/semantic-tables.js', array('wcpg-panel'), WCPG_VERSION, true);
        }
    }
    ...
    public function render($post_id) {
        ...
        // Stored edges are editable facts; derived ones reflect the
        // page/heading/item structure and render read-only below them.
        $stored  = array('outbound' => array(), 'inbound' => array());
        $derived = array('outbound' => array(), 'inbound' => array());
        foreach (array('outbound', 'inbound') as $direction) {
            foreach ($edges[$direction] as $edge) {
                if (!empty($edge['derived'])) {
                    $derived[$direction][] = $edge;
                } else {
                    $stored[$direction][] = $edge;
                }
            }
        }
        $edges = $stored;
        ...
        <section class="wcpg-panel" data-post-id="<?php echo esc_attr($post_id); ?>">
            ...
            <?php if (!empty($derived['outbound']) || !empty($derived['inbound'])) : ?>
            <details class="wcpg-derived">
                <summary>
                    <?php
                    printf(
                        /* translators: %d: number of structural connections */
                        esc_html__('structure (%d)', 'wcp-graph'),
                        count($derived['outbound']) + count($derived['inbound'])
                    );
                    ?>
                </summary>
                <ul class="wcpg-edge-list">
                    <?php foreach ($derived['outbound'] as $edge) : ?>
                        <li class="wcpg-edge wcpg-edge-derived">
                            <span class="wcpg-predicate"><?php echo esc_html($edge['predicate']); ?> →</span>
                            <a class="wcpg-chip" href="<?php echo esc_url($edge['object_url']); ?>"><?php echo esc_html($edge['object_title']); ?></a>
                        </li>
                    <?php endforeach; ?>
                    <?php foreach ($derived['inbound'] as $edge) : ?>
                        <li class="wcpg-edge wcpg-edge-derived">
                            <span class="wcpg-predicate"><?php echo esc_html($edge['inverse_label']); ?> →</span>
                            <a class="wcpg-chip" href="<?php echo esc_url($edge['subject_url']); ?>"><?php echo esc_html($edge['subject_title']); ?></a>
                        </li>
                    <?php endforeach; ?>
                </ul>
